...
for student in array :
    ...
    if len(candidates[0]) < n :
        # 3-1. 추천받은 학생의 사진 게시
        # 현재 추천받은 학생이 사진틀에 있는 경우
        if student in candidates[0] :
            candidates[1][candidates[0].index(student)] += 1
        # 현재 추천받은 학생이 사진틀에 없는 경우
        else :
            candidates[0].append(student)
            candidates[1].append(1)
    # 4. 비어 있는 사진틀이 없는 경우
    else :
        # 4-1. 현재 추천받은 학생이 사진틀에 있는 경우
        if student in candidates[0] :
            # 추천받은 횟수만 증가
            candidates[1][candidates[0].index(student)] += 1
        # 4-2. 현재 추천받은 학생이 사진틀에 없는 경우
        else :
            # 현재까지 추천받은 횟수가 가장 적은 학생의 사진 삭제
            min_recommendation = min(candidates[1])
            # 추천받은 횟수가 가장 적은 학생이 두 명 이상일 경우 처리
